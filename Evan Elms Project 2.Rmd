...
Note: The following columns cannot be used in predicting the estimate and will be initially removed before the analysis in each section:
...
addresTrain <- addressTrain %>% select(-bathrooms)
```
...
addresTrain <- addressTrain %>% select(-taxAssessmentYear,-regionType)
```
...
Note: Used the `filter` function to remove amounts greater than fourty million as there were less than ten outliers that made the plots hard to read due to the adjustments of the extreme home values. 
...
addresTrain <- addressTrain %>% select(-regionName)
```
...
addresTrain <- addressTrain %>% mutate(SingleFamily = ifelse(useCode == "SingleFamily",1,0)) %>% select(-useCode)
```
